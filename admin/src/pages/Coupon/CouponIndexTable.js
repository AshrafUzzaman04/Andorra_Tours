...
function CouponIndexTable() {
    ...
    const tableHeadings = [
        {
            ...
            sortable: true,
            reorder: true,
            selector: row => <NavLink to={`/coupons/${row.id}/edit`} >{row.id}</NavLink>
        },
        {
            name: t('Code'),
            sortable: true,
...

        {
            ...
            cell: row => <div className="text-center dropstart">
                ...
                <ul className="dropdown-menu">
                    <li>
                        <NavLink to={'/coupons/' + row.id + '/edit'} className="dropdown-item">
                            {t('Edit')}
                        </NavLink>
                    </li>
                    <li><hr className="dropdown-divider" /></li>
                    <li><a className="dropdown-item text-danger" href="#0" onClick={(e) => deleteAlert(e, 'coupons', row?.id, t).then(res => setRefresh(refresh + 1))}>{t('Delete')}</a></li>
                </ul>
            </div>
        }
    ];
    ...
    useEffect(() => {
        callFetch("coupons?page=" + pageNumber, "GET", []).then((res) => {
            setCoupons(res.data)
        });
    }, [pageNumber, refresh]);
...
export default CouponIndexTable