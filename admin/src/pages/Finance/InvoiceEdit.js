...
useEffect(()=>{
  if (editDelay == 1){
      if(params.id){
          callFetch("invoice/" + params.id + "/edit", "GET", []).then((res) => {
            // setValue('invoice_nr', res.data.invoice_nr);
            setValue('customer_name', res.data.customer_id);
            setValue('created_date',res?.data?.date)
            setValue('title',res.data?.title)
            setValue('description',res.data?.description)
              const product = JSON.parse(res.data.product_detail)
...
const onSubmit = (formData) => {
    ...
    callFetch("invoice/"+params.id, "POST", formData, setError).then((res) => {
        setSaving(false);
        if (!res.ok) return;
                                    ...
                                    <div key={i}>
                                        ...
                                        <div className="row mt-2">
                                            ...
                                            <div className="col-md-2">
                                                ...
                                                <input value={item.quantity === 1 ? 1 : item.quantity} type="number" className="form-control" placeholder={t("eg. 0.00")} onChange={(e) => {
                                                    let value = 0;
