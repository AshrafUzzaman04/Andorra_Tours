...
function PageTitleIndexTable() {
  ...
  const tableHeadings = [
    {
      ...
      sortable: true,
      reorder: true,
      selector: row => <NavLink to={`/theme-customization/page-category/${row.id}/edit`} >{row.id}</NavLink>
    },
    {
...

    {
      ...
      cell: row => <div className="text-center dropstart">
        ...
        <ul className="dropdown-menu">
          <li>
            <NavLink to={'/footer/page-category/' + row.id + '/edit'} className="dropdown-item">
              {t('Edit')}
            </NavLink>
          </li>
          <li><hr className="dropdown-divider" /></li>
          <li><a className="dropdown-item text-danger" href="#0" onClick={(e) => deleteAlert(e, 'page-category', row?.id, t).then(res => setRefresh(refresh + 1))}>{t('Delete')}</a></li>
        </ul>
      </div>
    }
  ];
  ...
  useEffect(() => {
    callFetch("page-category?page=" + pageNumber, "GET", []).then((res) => {
      setData(res.data)
    });
  }, [pageNumber, refresh]);
...
export default PageTitleIndexTable