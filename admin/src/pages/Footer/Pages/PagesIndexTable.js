...
function PagesIndexTable() {
  ...
  const tableHeadings = [
    {
      ...
      sortable: true,
      reorder: true,
      selector: row => <NavLink to={`/footer/pages/${row.id}/edit`} >{row.id}</NavLink>
    },
    {
...

    {
      ...
      cell: row => <div className="text-center dropstart">
        ...
        <ul className="dropdown-menu">
          <li>
            <NavLink to={'/footer/pages/' + row.id + '/edit'} className="dropdown-item">
              {t('Edit')}
            </NavLink>
          </li>
  ...
  useEffect(() => {
    callFetch("footer-pages?page=" + pageNumber, "GET", []).then((res) => {
      setData(res.data)
    });
  }, [pageNumber, refresh]);
...
export default PagesIndexTable