...
function SocialLinkIndexTable() {
  ...
  const tableHeadings = [
    {
      ...
      sortable: true,
      reorder: true,
      selector: row => <NavLink to={`/footer/social-links/${row.id}/edit`} >{row.id}</NavLink>
    },
    {
      name: t('Icon'),
      sortable: true,
      reorder: true,
      selector: row => row.social_icon
    },
    {
        name: t('Link'),
        sortable: true,
...

    {
      ...
      cell: row => <div className="text-center dropstart">
        ...
        <ul className="dropdown-menu">
          <li>
            <NavLink to={'/footer/social-links/' + row.id + '/edit'} className="dropdown-item">
              {t('Edit')}
            </NavLink>
          </li>
          <li><hr className="dropdown-divider" /></li>
          <li><a className="dropdown-item text-danger" href="#0" onClick={(e) => deleteAlert(e, 'footer-social-link', row?.id, t).then(res => setRefresh(refresh + 1))}>{t('Delete')}</a></li>
        </ul>
      </div>
    }
  ];
  ...
  useEffect(() => {
    callFetch("footer-social-link?page=" + pageNumber, "GET", []).then((res) => {
      setData(res.data)
    });
  }, [pageNumber, refresh]);
...
export default SocialLinkIndexTable 