...
function CamerasIndexTable() {
  ...
  useEffect(() => {
    if (refresh === 0) {
      callFetch("webcams", "GET", []).then((res) => {
        setData(res?.data);
        setRefresh(1)
      });
    }
            ...
            <div className="card shadow-sm overflow-hidden">
              <div className="card-header p-3 d-flex align-items-center justify-content-between" style={{ backgroundColor: "#F8F9FA" }}>
                ...
                <div className="d-flex align-items-center gap-2">
                  <NavLink to={"/others/cameras/" + item?.content_for + "/edit"}><i className="fa fa-edit cursor-pointer"></i></NavLink>
                </div>
              </div>
...
export default CamerasIndexTable