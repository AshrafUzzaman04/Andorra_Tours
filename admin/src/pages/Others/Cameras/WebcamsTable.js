...
function WebcamsTable() {
...

    {
      ...
      cell: row => <div className="text-center dropstart">
        ...
        <ul className="dropdown-menu">
          <li>
            <NavLink to={'/others/webcams/' + row.id + '/edit'} className="dropdown-item">
              {t('Edit')}
            </NavLink>
          </li>
          <li><hr className="dropdown-divider" /></li>
          <li><a className="dropdown-item text-danger" href="#0" onClick={(e) => deleteAlert(e, 'webcams', row?.id, t).then(res => setRefresh(0))}>{t('Delete')}</a></li>
        </ul>
      </div>
    }
  ];
  ...
  useEffect(() => {
    if (refresh === 0) {
      callFetch("webcams?page=" + pageNumber, "GET", []).then((res) => {
        setData(res.data);
        setRefresh(1)
      });
    }
