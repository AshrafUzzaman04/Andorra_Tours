...
const OfferBannerEdit = () => {
  ...
  useEffect(() => {
    if (params?.id) {
      callFetch("promotion/" + params.id, "GET", []).then((res) => {
  ...
  const onSubmit = (formData) => {
    setSaving(true);
    callFetch("promotion/"+params.id, "POST", formData, setError).then((res) => {
        setSaving(false);
        if (!res.ok) return;
...
export default OfferBannerEdit