...
function ServicesIndexTable() {
  ...
  const tableHeadings = [
    {
      ...
      sortable: true,
      reorder: true,
      selector: row => <NavLink to={`/theme-customization/servcios-exclusivos/${row.id}/edit`} >{row.id}</NavLink>
    },
    {
      ...
      selector: row => <div className="row mt-1 mb-1 d-flex align-items-center" style={{ width: '600px' }}>
        <div className="col-2 pe-0">
          <img className="avatar avatar-sm" src={row?.service_image ? process.env.REACT_APP_STORAGE_URL + row?.service_image : '/assets/img/placeholder.png'} alt="photo" />
        </div>
      </div>
    },
    {
    ...
    {
        name: t('Total Services'),
        sortable: true,
        reorder: true,
        selector: row => row.total_services
      },
    {
      name: t('Service Link'),
      sortable: true,
  ...
  useEffect(() => {
    callFetch("service?page=" + pageNumber, "GET", []).then((res) => {
      setData(res.data)
    });
  }, [pageNumber, refresh]);
