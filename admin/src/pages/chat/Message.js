...
function Message() {
    ...
    useEffect(() => {
        if (echo) {
            echo.private(`messages.${user?.id}`).listen('RetriveMessages', event => {
                if (event.receiver.id === user?.id && event.sender.id === Number(id)){
                    handleAutoScroll();
                    callFetch("messages/" + id, "GET", []).then((res) => {
                                            ...
                                            <ul className="list-unstyled chat-list chat-user-list">
                                                {!loading && (
                                                    participations?.map((user) => (
                                                        <li className={id == user?.user.id ? 'active' : ''} onClick={handleResponsive}>
                                                            <NavLink to={'/chat/message/' + user?.user.id}>
                                                                <div className="d-flex align-items-center">
                                                                    <div className="chat-user-img online align-self-center me-3 ms-0">
                                                                        <img className="avatar avatar-sm object-fit" src={user?.user.photo ? process.env.REACT_APP_STORAGE_URL + user?.user.photo : '/assets/img/placeholder.png'} alt="" />
                                                                        {user?.status === 1 && <span className="user-status"></span>}
                                                                    </div>
                                                                    ...
                                                                    <div className="dropdown align-self-start">
                                                                        ...
                                                                        <div className="dropdown-menu">
                                                                            <a className="dropdown-item" href="javascript::void(0)" onClick={() => deleteChat(user?.user.id)}>Delete <i class="ri-delete-bin-line float-end text-muted"></i></a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </NavLink>
                                                ...
                                                {loading && (
                                                    Array.isArray(searchResult) && searchResult?.map((user) => (
                                                        <li key={user?.id} className={id == user?.user ? 'active' : ''}>
                                                            <NavLink to={'/chat/message/' + user?.user}>
                                                                <div className="d-flex align-items-center">
                                                                    <div className="chat-user-img online align-self-center me-3 ms-0">
                                        ...
                                        <div className="row g-0">
                                            <div className="col-12">
                                                <div className="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        {!saving && (
                                                            <button type="submit" className="btn btn-primary ms-2" id="sendMessageBtn">
                                                                {t("Send")}
                                                            </button>
                                                        )}
                                                        {saving && (
                                                            <button type="submit" className="btn btn-disabled ms-2" disabled>
                                                                {t("Sending")}
                                                            </button>
                                                        )}
                                                    </div>
