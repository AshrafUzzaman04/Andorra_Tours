...
function CustomerEdit() {
    ...
    useEffect(() => {
        if (editDelay == 1)
            callFetch("customers/" + params.id + "/edit", "GET", []).then((res) => {
                for (let [key, value] of Object.entries(res.data)) {
                    setValue(key, value);
                }
                let cp = JSON.parse(res.data.contact_person);
                setContactPersons(cp);
                setLangLat({latitude: res.data.latitute, longitude:res.data.longitute});
            });
    }, [editDelay,params.id]);
    ...
    const onSubmit = (formData) => {
        ...
        callFetch("customers/" + params.id, "POST", formData, setError).then((res) => {
            setSaving(false);
            if (!res.ok) return;
