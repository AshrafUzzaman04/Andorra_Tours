...
const CustomerBaseProjectTable = ({employeeId}) => {
    ...
    useEffect(() => {
        if(employeeId > 0){
            callFetch("customer-project/"+ employeeId ,"GET", []).then((res) => {
    ...
    useEffect(()=>{
        if(employeeId > 0){
            if(searchKey.length > 0){
                callFetch('employeebase/'+searchKey+'/'+employeeId, "GET", []).then((res)=>{
                    setTableData(res.data)
                })
...
export default CustomerBaseProjectTable