...
const ImportModal = (props) => {
    ...
    const onSubmit = (formData) => {
        ...
        callFetch("import/suppliers/","POST",formData,setError).then((res) => {
            setSaving(false);
            setSuccess(res.data)
        ...
        <form className={`needs-validation ${Object.keys(errors).length ? "was-validated" : ""}`} onSubmit={handleSubmit(onSubmit)} noValidate autoComplete="off">
            ...
            <div className="modal-body">
                <SoftTypography variant="caption" color="text" >
                    ...
                    {/* <a className="ms-2 text-blue-700" href={File} download="Supplier">CSV,</a> */}
                    <a className="ms-2 text-blue-700" href={FileXLSX} download="Supplier">XLSX</a>
                </SoftTypography>
                <hr />
                {
                ...
                <div className="drop_box">
                    ...
                    <label htmlFor="fileID" className="btn">{t('Choose File')}</label>
                </div>
            </div>
