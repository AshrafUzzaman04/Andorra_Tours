...
export default function BookingForm({
  ...
  async function handleBooking() {
    ...
    try {
      ...
      if (
        bookingReantData.price > 0 &&
        bookingReantData.startDate &&
        bookingReantData.endDate
      ) {
        setTimeout(() => {
          setBookingNow(false);
        }, 500);
        await router.push("/cart/reant");
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const calculateDayCount = async () => {
      ...
      const res = await Fetch.post("/product/price", {
        id: FormData?.id,
        for: FormData?.product_for,
        day: day,
      });
      ...
      const totalPrice =
        ...
        calculateTotalPrice(quantities, selectedExtras);
    ...
    if (
      ...
      const calculateDayCount = async () => {
        ...
        setbookingReantData((prev) => ({
          ...prev,
          day: day,
          price: price?.online_price ? totalPrice : 0 + calculateTotalPrice(),
        }));
      };
      calculateDayCount();
    }
  ...
  const handleQuantityChange = (i: number, value: number) => {
    ...
    const totalPrice =
      Number(dayPrices?.online_price) *
        (totalQuantity !== 0 ? totalQuantity : 1) +
      calculateTotalPrice(updatedQuantities, selectedExtras);
  ...
  const calculateTotalPrice = (
    ...
    const extrasTotal =
      totalQuantity > 0
        ? parsedAddExtras?.reduce(
            (sum, extra, i) =>
              sum +
              (updatedSelectedExtras[i]
                ? Number(extra.price) *
                  (totalQuantity !== 0 ? totalQuantity : 1)
                : 0),
            0
    ...
    <>
      <div className="content-booking-form">
                ...
                <p className="text-xl-bold neutral-1000">
                  {dayPrices?.online_price ? (
                    <div>
                      <NumericFormat
                        value={
                          bookingReantData.price ?? dayPrices?.online_price
                        }
                        displayType="text"
                        thousandSeparator={","}
                        decimalSeparator="."
        ...
        <div
          ...
          onClick={() => {
            bookingLink === null && handleBooking();
          }}
        >
          {" "}
          <Link
            className="btn btn-book"
            href={bookingLink !== null ? bookingLink : "#"}
          >
            {bookingNow && (
              <div
                style={{
